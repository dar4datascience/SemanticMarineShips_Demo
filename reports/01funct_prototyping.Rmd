...
get_ship_names(Vessel_types_n_names, sample_ship_t_n$ShipType[4])
...
calc_circle_distance <- function(spatial_df) {
  if (class(spatial_df)[1] != "sf") {
    ...
    lagged_df <- spatial_df %>%
      lag()
    
    df_distances <- tibble(
      DATETIME = spatial_df$DATETIME,
      distance_travelled =  func_distance(from = spatial_df, to = lagged_df)
    )
...
get_max_distance <- function(df_distances) {
  if (colnames(df_distances)[2] != "distance_travelled") {
...
record_top_distance_points <- tibble(
  ...
  df %>% 
    filter(DATETIME == top_distance$DATETIME)
  ) %>%
      st_as_sf(.,
               coords = c("LON", "LAT"),
               crs = 4326)
...
lagged_kerli <- sf_kerli %>% arrange(DATETIME) %>% lag()
...
kerli_distances <- tibble(
  DATETIME = sf_kerli$DATETIME,
  distance_travelled =  func_distance(from = sf_kerli, to = lagged_kerli) # if TRUE, return a vector with distance between the first elements of x and y
) %>% 
  slice_max(order_by = distance_travelled)
...
##### see some points in the map

```{r mapviewSample}
library(mapview)

mapview(top_record_kerli_distance)
```
